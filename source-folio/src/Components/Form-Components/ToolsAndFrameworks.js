  ...
  const toolLevelOptions = ["Expert", "Intermediate", "Beginner"];
    return (
        ...
        {inputList.map((x, index) => {
            return (
                <div className="border border-gray-900 rounded-lg p-2 mt-2">
            <InputBox field="Tool/Framework Name" type="text" id={`toolName_${index}`} index = {index}  name="toolName" value={x.toolName} handleChange={handleinputchange} placeholder="Enter tool/framework name"></InputBox>
            <CustomSelect field="Tool/Framework Level" index={index} value={x.toolLevel} name="toolLevel" id={`toolLevel_${index}`} array={toolLevelOptions} handleChange={handleinputchange}></CustomSelect>
